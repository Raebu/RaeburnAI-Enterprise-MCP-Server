...
export function createEnterpriseMcpServer(context: ConnectorContext, auditLog: AuditLog): McpServer {
  ...
  server.resource('connectors', 'raeburnai://connectors', async () => ({
    contents: [{ uri: 'raeburnai://connectors', mimeType: 'application/json', text: JSON.stringify(connectorStatus(context), null, 2) }]
  }));

  server.tool('enterprise.health', 'Return server health and available tools.', {}, async () => ({
    content: [{ type: 'text', text: JSON.stringify({ status: 'ok', ...connectorStatus(context), tools: allTools(context).map((item) => item.name) }, null, 2) }]
  }));
...
function registerTool(server: McpServer, enterpriseTool: EnterpriseTool, context: ConnectorContext, auditLog: AuditLog) {
  const schema = zodToJsonSchema(enterpriseTool.inputSchema, enterpriseTool.name) as Record<string, unknown>;
  const shape = 'properties' in schema ? (schema.properties as Record<string, unknown>) : {};

  server.tool(enterpriseTool.name, enterpriseTool.description, shape, async (input: unknown) => {
    ...
    if (!policy.allowed || policy.approvalRequired) {
      auditLog.record({ tool: enterpriseTool.name, connector: enterpriseTool.connector, risk: enterpriseTool.risk, input, status: policy.allowed ? 'approval_required' : 'blocked', reason: policy.reason });
